...
/* Compute e=(x-y)/var for three n-vectors x, y and var and return the squared L2 norm of e.
 * e can coincide with either x or y; x can be NULL, in which case it is assumed
 * to be equal to the zero vector.
 * Uses loop unrolling and blocking to reduce bookkeeping overhead & pipeline
 * stalls and increase instruction-level parallelism; see http://www.abarnett.demon.co.uk/tutorial.html
...
LM_REAL LEVMAR_L2NRMXMY_VAR(LM_REAL *e, LM_REAL *x, LM_REAL *y, LM_REAL *var, int n, LM_REAL varThreshold)
{
  int i;
  LM_REAL sum=0.0;
  ...
  if (var)
  {
    ...
    for(int i=0; i<n; ++i)
    {
      e[i] = (var[i] < varThreshold) ? 0 : (x[i] - y[i]) / var[i];
      sum += e[i]*e[i];
    }
